...
int main()
{
    ...
    for (int i_sim = 0; i_sim < n_sim; i_sim++)
    {
        ...
        w_lcs = w_lcs + (I_lcs.inverse()*((Vec3)(I_lcs*w_lcs)).cross(w_lcs))*h;

        //convert angular velocity to global frame
        w_gcs = R_g2l*w_lcs;

        //compute rotation matrix using Rodriguez's formula
        K = w_gcs.normalized().skew();
        angle = w_gcs.norm()*h;
        rot = eye + K*sin(angle) + K*K*(1-cos(angle));

        //update orientation by w_gcs * dt
        R_g2l = rot * R_g2l;

        //update w_lcs
        w_lcs = R_g2l.transpose()*w_gcs;
